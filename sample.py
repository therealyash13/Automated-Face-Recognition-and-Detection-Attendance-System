...
def TakeImages():
    ...
    if (is_number(Id) and name.isalpha()):
        ...
        row = [Id, name]
        with open('StudentDetails\StaffDetails.csv', 'a+') as csvFile:
...
def TrackImages():
    ...
    df = pd.read_csv("StudentDetails\StaffDetails.csv")
    cam = cv2.VideoCapture(0)
...
takeImg = tk.Button(window, text="Sample",
                    command=TakeImages, fg="Red", bg="green",
                    width=20, height=3, activebackground="Red",
                    font=('times', 15, ' bold '))
takeImg.place(x=200, y=500)
trainImg = tk.Button(window, text="Training",
                     command=TrainImages, fg="Red", bg="green",
                     width=20, height=3, activebackground="Red",
                     font=('times', 15, ' bold '))
trainImg.place(x=500, y=500)
trackImg = tk.Button(window, text="Testing",
                     command=TrackImages, fg="Red", bg="green",
                     width=20, height=3, activebackground="Red",
                     font=('times', 15, ' bold '))
